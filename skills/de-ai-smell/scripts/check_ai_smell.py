...
分账/结算账期/账务)负向排除不误报。
含 2026-08-28 用户定规:「翻旧账」并入记账类比 L1(评论区回复被打回,账务色彩比喻一律不要)。
...
PATTERNS: dict[str, re.Pattern] = {
    ...
    "有机生长隐喻(2026-08-28用户定规)": re.compile(r"(?<![增生成])长成了|(?<![增生成])长在(文件|清单|台账|代码|仓库|流程|里面|里)|从事故里长|从坑里长"),
...
LABEL_TIPS = {
    "记账类比(X的账/这笔账/算账)": (
        ...
        "「这笔账划算」→「值得」;「翻旧账」→「查旧记录/翻早前的日志」;"
        "字面义(账单/账号/账户/分账/对账)不受限"
    ),
}
...
SUGGEST = {
    ...
    "赋能": "换具体动作「帮X做成什么」",
    "闭环": "描述具体回路才留,泛指删",
    "长成了": "有机生长隐喻(2026-08-28禁):改「搭建/沉淀沉淀不用/攒成/发展成」;字面树木生长可留",
    "长在": "有机生长隐喻(2026-08-28禁):改「写在/存于/落在」;字面位置可留",
    "从事故里长": "有机生长隐喻:改「从事故里攒出/从事故换来」",
    "抓手": "换「具体手段/方法」",
    "打通": "换「连上/对接」",
    "拉通": "换「协调」或删",
    "洞察": "换「发现/结论」",
