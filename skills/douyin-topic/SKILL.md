...
- 在 `topics.md` / `rough_outlines/` 看中某条 → **先跑 `/media-review` 选题预测（1 万线门禁，2026-09-04 定规）：六维预估中位数 ≥10,000 才算选题完成**，不达线按四杠杆迭代 ≤3 轮 → `make topic-deep id=<group_id>` 确认后深挖（Phase 2）
...
## 自家爆款快反（hit fast-follow，2026-08-30 定规）

爆款出现后的 48h 续拍流水线——数据源自动触发，不靠人记：

- **触发**：`make next` 的 7d 转化判断报「≥5000 达爆款线，评估加拍同题材」或 timeseries.db 爆款标记（默认抖音播放 ≥5000，人工确认）——出现即进本流程，不当普通选题回流处理
- **拆解对象 = 自家爆款**（不是外部热榜）：复用 Phase 2 拆解管线，但原片就是自己——钩子前 3s 话术、段落时间轴、**自家评论区追问**（自己热评区的问题就是现成的续拍选题清单，这部分外部对标给不了）
- **续拍角度三选一**：同题材加深（下钻其中一个机制）/ 续集（epN+1，接上集结尾悬念）/ 坑位补全（评论区集中问的翻车点）
- **快反窗口 48h**：爆款流量池还在推——确认选题当天下大纲、次日进渲染、第三天内挂定时；**双窗口 20:00 定规不动**，队列冲突照常顺延；超 48h 未进生产则放弃赶窗，回常规排期（热度衰减后快反无意义）
- **登记**：`make topic-funnel ARGS="add ..."`（来源标 `fast-follow`）+ `vpt stage <slug> backlog`；系列命名沿用 epN（dsh-source-deep-dive 系列即本流程首个实战对象）
