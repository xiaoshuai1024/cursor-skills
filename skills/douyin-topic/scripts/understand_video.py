...
# Adapted from OpenMontage video-understand (AGPL-3.0).
# 本文件按 AGPL-3.0 单独许可（区别于本仓库其余部分的 MIT）；再分发须遵循 AGPL-3.0。
# 本仓改造：转写改 --transcribe 显式开启（对标拆解的语音转写走自有 transcribe.py，
# 本脚本只承担镜头级结构：场景切分 + 关键帧帧表），去除交互式自动安装。

"""understand_video — 对标视频镜头级拆解（ffmpeg 场景切分 + 关键帧帧表，零 API key）。

转写回答「说什么」，本脚本回答「怎么拍」：镜头数/切点/每镜时长/代表帧图。
whisper 转写仅在显式 --transcribe 时执行（默认不装 openai-whisper 也能跑）。

Examples:
    python understand_video.py video.mp4                      # 场景切分帧表（默认）
    python understand_video.py video.mp4 -m keyframe          # I 帧模式
    python understand_video.py video.mp4 -m interval --max-frames 10   # 均匀采样
    python understand_video.py video.mp4 --transcribe         # 附带 whisper 转写
    python understand_video.py video.mp4 -q -o result.json    # 静默 + JSON 落盘
"""
...
def _check_and_offer_install():
    ...
    log("  (or skip: the teardown pipeline transcribes via its own transcribe.py)")
...
def understand_video(video_path, mode="scene", max_frames=_DEFAULT_MAX_FRAMES,
                     whisper_model=_DEFAULT_WHISPER_MODEL, do_transcribe=False):
...
def build_parser():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""\
examples:
  %(prog)s video.mp4                          Scene detection frame map (default)
  ...
  %(prog)s video.mp4 --transcribe             Also transcribe via whisper
...
""",
    ...
    parser.add_argument(
        "--transcribe",
        action="store_true",
        help="Explicitly enable whisper transcription (default off; "
             "the teardown pipeline transcribes via its own transcribe.py)",
    )
    parser.add_argument(
        "-o", "--output",
        metavar="FILE",
