...
- **SEO 长尾思维**:标题 = 关键词 + 意图,技术关键词放前 8 字,抓眼球词紧跟其后
...
**约束一:结构门禁 + 篇幅分档**。发布门禁是结构完整度(本类型必需块齐全),不是字数。篇幅按主题规模分档:小 2000–4000 / 中 4000–7000 / 大 7000–12000 字(正文中文字符,不含 front matter / 代码块 / 图注),以讲透为准。写完 <2000 → 选题太薄或结构缺块,回选题关 / 补结构,不是凑字。
...
跑这个脚本,一次性检查字数、配图、SVG 完整性(Windows 上 `python3` 可能不解析,用 `python`;务必带 `PYTHONIOENCODING=utf-8`,否则打印中文 / emoji 报 GBK 错):
...
# 硬门禁:配图配额 + SVG 完整性(结构门禁是人工 checklist,不程序量化)
band = '小(2000-4000)' if cn<4000 else ('中(4000-7000)' if cn<7000 else '大(7000-12000)')
quota = 2 if cn<4000 else (3 if cn<7000 else 5)
ok = imgs>=quota and not missing
warn = ' ⚠️ 不足2000:选题太薄或结构缺块,回选题关/补结构' if cn<2000 else ''
print(f\"{'✅' if ok else '❌'} {slug}: {cn}字[{band}] / {imgs}图(需≥{quota})\" + warn + (f' 缺失SVG:{missing}' if missing else ''))
"
```
...
- [ ] 结构门禁:本类型必需块齐全(非日常/观点型);篇幅落在主题规模档位,<2000 回选题关
...
字数验证用 `[\u4e00-\u9fff]` 只数中文,英文术语和标点不计。这是**最严格**计数——纯中文都过门槛,加英文只会更多。别用「含英文」的宽松计数自欺欺人。

### 去 AI 味会削字,完成后要复验

删「回到本质」段平均削 300-400 字。**每次去 AI 味后都要重跑字数脚本**,确保仍落在合理档位——削到 2000 以下就检查是不是结构缺块,补有信息量的内容,不是补套话。
...
- **篇幅规则以 `references/post-types-guide.md` 为准**:结构门禁 + 篇幅分档,不以固定字数为合格线
