...
**约束一:结构门禁 + 篇幅由内容定(2026-08-26 取消各类文章字数限制)**。发布门禁是结构完整度(本类型必需块齐全)+ 内容讲透,不是字数——各类文章的字数下限与篇幅分档带已全部作废。定篇幅用**观点清点法**(详见 `references/post-types-guide.md` 总则):列观点清单 → 每个观点问「读者现状 / 给什么证据 / 讲到什么程度算懂」→ 展开量加总即篇幅。短得有理不凑字;写完明显偏短 → 先对照结构清单查缺块(步骤没讲透 / 弯路没写全),查不出缺块就是内容本来这么小,不回炉凑字。
...
**E. 封面**
- [ ] **首图即公众号封面**(prepare.py 自动取首图 9:5 裁切):首图够醒目、小图下文字可读、不是纯代码截图
...
- [ ] 同步公众号:过 `references/wechat-retention.md` 留存清单(`wechat_title`/`wechat_digest` 变体、首屏、节奏、扫读、钩子兑现、**收藏触发**(至少一处可收藏资产)、**文末往期关联 2-3 篇**、转化段不进源稿)
...
- [ ] `hugo build` 成功;公众号发布 48h 回看已排期(**对照完读率 30/50/65 基准线归档**,打开低修标题摘要、完读低修首屏节奏,数据结论写 link-map.json 备注)
