...
- **发布执行（无顺序约束）**：四平台同窗口挂齐，串行跑。快手走 v2（`py -3.11 scripts/pub/kuaishou_publish_v2.py <slug> "YYYY-MM-DD HH:MM"`）；B站 `--platforms bilibili`（biliup dtime 平台侧定时）；抖音 `--platforms douyin`；视频号 `--platforms shipinhao`（**2026-09-06 用户定规：一律平台原生定时，跨日档可用，schtasks 直发退役禁用**——旧「仅 ~24h 近档/跨日静默降级」结论（2026-08-28 四次实验）经 2026-09-06 实证推翻：前一天上午挂次日 20:00 跨日档一次成功，audit「⏳待发表定时卡」在列；此前 09-05 两次「日历未选中中止」真因为登录态退化而非平台限制，处置 = 扫码续登 cookie 后重试。发布后仍 MUST pub-audit 复核「待发表定时卡」在列，防静默回退（红线 6））。
...
- **发布前投放节拍：官方活动 + 热门话题（2026-08-31 用户定规，openspec post-publish-ops-explore）**：每次发布作品时，同步对各平台热门活动和话题做匹配投放——发布前扫抖音创作中心首页活动中心/热门话题（活动规则与截止日查活动日历 `creator-micro/creative-guidance/calendar`）与 B站 member 首页热门活动，**题材匹配即挂**：抖音在**发布页「官方活动」勾选区**勾选（原生参投通道；发布页与编辑页同款勾选器）+ 话题行长尾槽换当期热门话题；B站上传页「参加活动」；快手/视频号活动位出现即查。**抖音话题与标签发布后不可改、每作品限改 5 次——发布时一次挂对**。投稿前过 platform-compliance 扫描；活动规则原文在 confirm 门禁时向用户复述（如「人文经典AI」征稿实为「aigc 人文经典作品」窄主题，边缘匹配须用户点头）。已挂账：抖音「千万流量！人文经典AI作品等你投稿」（08.15-09.30）、B站「AI支个招 社团请就位」（风控冷却后随下次发布）。不做直播、不投流（付费加热一律不碰）；免费活动不受限。运营位全景与挂起门槛见 **post-publish-ops skill**（发布后统一入口）。
...
- ~~**兜底**：平台侧定时控件失效时，用 Windows 任务计划程序本地直发落 20:00 窗口~~ **2026-09-06 用户定规：本地直发退役禁用**（「不要直发,使用平台定时功能」）。平台定时控件失效（「日历未选中中止」等）的处置 = 扫码续登 cookie → 重试平台定时；仍失败上报用户，不得回退 schtasks/bat 直发。遗留 sph-* 任务已全部拆除（2026-09-06：sph-publish-2000 / sph-w1-gpt6-astra-coding-0906 / sph-w1-gpt6-astra-impact-0906；sph-publish-2000 经查为 08-27 一次性遗留，仅触发过一次，无重复发布堆积）。
- ~~待修：tencent 定时选择器~~ **2026-08-28 已重写修复**：旧 `label.nth(1)` 只搜主 frame + `query_selector_all` 穿不透 Shadow DOM（面板全在主 frame Shadow DOM 里，v4-v9 探针实证）。新实现（`tencent_uploader_main.py::set_schedule_time_tencent`）：全 frame locator 探测定时 radio → 日历选日（跨月翻页+日期回读验证）→ 时/分转盘点选（虚拟列表 hover+滚轮推进、按列 x 坐标分左右防串列）→ 主输入框全串 `YYYY-MM-DD HH:MM` 终验；`submit_publish` 加终防线：scheduled 策略下未验证挂上即拒绝点发表，宁可失败不可误发。配套：原创分成提示弹窗（Shadow DOM）改 locator 显式点「直接发表」（AI 内容不声明原创）；`error_capture` 修复「不超过」误命中「超过」。**残余限制（2026-09-06 修订）**：~~跨日（>~24h）定时平台侧静默降级立即发表,跨日档必须 schtasks 直发~~——该结论已实证作废（跨日 34h 档一次成功,「静默降级」实录系登录态退化期样本）；发布后 MUST pub-audit 复核「⏳待发表定时卡」在列防回退。仍在的限制：AI 声明开关与 4:3 横版封面 UI 超时跳过（出片后后台手动补）。
...
**概念定规（2026-08-30 用户定规，2026-09-06 修订）**：「定时发布」**专指使用平台自身的定时发布功能**——抖音发布页定时控件、快手「定时发布」、B站 `dtime` 定时投稿、视频号发布页定时控件。本地 schtasks 挂 bat 定时直发**不属于定时发布**，称「本地兜底直发」（术语保留仅作历史表述）。台账（state.json/link-map）、汇报、审计中两者必须分开表述，四平台齐全判定不受影响。~~视频号平台定时实证仅当天内近档（小时级）可用,>当天 档位只能本地兜底~~——**2026-09-06 实证推翻并用户定规：跨日档平台定时可用，schtasks 本地兜底直发退役禁用**，任何档位一律平台定时；旧「15:50 档降级实录 + ep3 canary 78h 必降」样本均系登录态退化期采集，主因为 cookie 而非平台窗口。
...
- **视频号——无频控实证**：通道改平台原生定时（2026-09-06 用户定规，schtasks 直发退役禁用），频控风险目前为零。
