...
1. **全元素动画（硬性）**：画面中每个内容元素——标题/副标题/标题条/要点/知识卡/流程图节点与连线/代码高亮/图表数字/字幕/进度条/封面 outline——都必须有**入场、换态、强调三类动画至少其一**；任何镜头静态停留超限（硬上限 8s，见下方「任意镜头静置上限」）= 违规返工。静态信息轮播是反例（2s 跳出率 >45% 的根因之一）。
   - **任意镜头静置上限（2026-09-05 用户再定规，全片通则不限首屏）**：任何静态画面都不要长时间停留——用户判断「长时间静态停留会造成用户流失」，经数据核对成立（本仓：GPT-6 首屏 23s 静态实录、2s 跳出率 >45% 根因含静态标题页；外部：72% 观众前 3s 决定去留、留存剪辑通行线每 2-3s 一次可见变化）。执行口径：
     - **分层数字（2026-09-05 用户定规三次收紧，替代原 ≤10s 静态线与 ≤15s 镜头线）**：
       - 镜头内**可见信息变化间隔：≤3s 目标、≤5s 硬线**（字幕意群推进/高亮切换/要点亮起/微动均计；钩子段把分句拆短可压到 2-3s）；
       - **讲解镜头 4-8s**（硬上限 8s），**停留 >5s 必须安排一次换态强调**（高亮切换/要点亮起/圈注/token 形变/数字滚动任一），出生帧管线天然提供节拍锚；
       - **快节奏镜头（枚举/榜单/氛围/转场）2-4s，最短 2s**；
       - **讲解镜头最短 ≥3s 且必须覆盖至少一个完整意群**（分句 4-6s——不许把一句话劈到两个镜头，三同步优先于节奏）；
       - **前 30s 黄金段：镜头 ≤5s、信息变化 ≤3s**（节奏前紧后松）；
     - **静态期底线**：即便在讲解核心要点（豁免「无意义停留」判定），常驻微动（呼吸辉光/进度条/极光漂移）+ 字幕推进必须在场——全片不存在完全静止的画面；
     - **判据是「可见信息变化」不是「大幅运动」**：字幕推进、高亮切换、笔迹生长都算数——观众阅读代码/表格需要画面稳定，禁为动而动（Mayer 相干性原理：无关动效损害理解）；动效必须挂讲解节拍，无语义乱动同样是违规。
...
3. **节奏门禁（硬规则，2026-09-05 收紧）**：任何镜头停留 **≤8s**（快节奏镜头 2-4s，讲解镜头 4-8s）；卡 >24s ≥3 镜头、16-24s ≥2 镜头、≤16s 1-2 镜头；`from_s` 必须对齐该卡口播**句边界**（`boundaries_*.json` 的 start_ms），禁止句中切；占位符（「讲解中…」）禁止出现在成片。写 deck 后跑节奏自检（镜头 span + 数量下限）。
