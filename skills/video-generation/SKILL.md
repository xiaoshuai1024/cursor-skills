...
**2026-09-05 用户定规（首屏与镜头静置，违者返工）**：任何镜头不得长时间停留在静态内容——除非该镜头已展示核心要点且正在讲解中。**同日用户再定规（升格全片通则）：任意一个静态画面都不要长时间停留**（长期静态停留=用户流失源，数据核对成立）——硬线任一镜头静态 ≤10s；静态期内常驻微动+字幕推进必须在场（全片无完全静止画面）；停留 >6s 安排一次换态强调；判据是「可见信息变化」而非「大幅运动」，阅读型画面（代码/表格）保稳定、动效必挂讲解节拍。执行口径见 `references/animation-shots.md` 规范①。**intro 卡同样受 >10s 纯静态禁令约束：首屏必须带核心要点与镜头轮换（纯标题静态首屏 = 返工，2026-09-05 GPT-6 片首屏 23s 静态实录）**。工程坑：deck 卡 `points` 为空会触发 builder `is_cover` 判定并**剥离该卡全部 shots**（`is_cover = bool(card.get("is_cover")) or len(points_raw)==0`）——intro 卡要上镜头必须先给 `points`。
