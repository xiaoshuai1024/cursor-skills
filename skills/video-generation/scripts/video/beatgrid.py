...
# Adapted from OpenMontage music-to-video/scripts/analyze-beatgrid.py (AGPL-3.0).
# 本文件按 AGPL-3.0 单独许可（区别于本仓库其余部分的 MIT）；再分发须遵循 AGPL-3.0。方法论见 references/beat-cut.md。

"""beatgrid — BGM 节拍网格 + 能量叙事分析器（beat-cut 卡点能力的唯一时间事实源）。

把任意 BGM 轨道确定性地转成 `audiomap.json`——画面卡点挂靠的音乐骨架。合并：

  - 可信的 tempo + 节拍网格 + downbeat（librosa beat tracker），
  - 每事件的节拍位置（strong / weak / syncopated / off-grid，16 分音符小节网格），
  - 鼓件分类（kick / snare / hihat / perc，带通拆分启发式），
  - 特殊音频事件（riser / glitch / crash-impact / hard-stop 静默），
  - 能量叙事（音频驱动的能量相位 + builds + key moments；段落名由分析自定，无固定模板），
  - 短语层 + 分段密度预算（视觉规划用）。

使用：
    python beatgrid.py bgm.mp3 -o audiomap.json
...
何时是节拍器幻觉）见 references/beat-cut.md。
...
except ImportError as _exc:  # 给出可执行的补救，不裸 traceback
    sys.stderr.write(
        "[beatgrid] 缺依赖: %s\n" % _exc
