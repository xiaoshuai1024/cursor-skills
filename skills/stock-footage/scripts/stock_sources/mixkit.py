...
# Adapted from OpenMontage (github.com/calesthio/OpenMontage, AGPL-3.0).
    ...
    def download(self, candidate: Candidate, out_path: Path) -> Path:
        ...
        try:
            r = requests.get(
                ...
                # 2026-08 实测：bot UA 会被降级页面（无 mp4 直链），需浏览器 UA
            ...
            # Fast path (2026-08 实测): asset 直链直接在页面 HTML 里，规律
            # https://assets.mixkit.co/videos/<id>/<id>-720.mp4（另有 -360）。720 优先。
            import re as _re
            _asset_urls = _re.findall(
                r"https://assets\.mixkit\.co/videos/[^\"']+?\.mp4", r.text)
            if _asset_urls:
                download_url = next(
                    (u for u in _asset_urls if u.endswith("-720.mp4")), _asset_urls[0])
