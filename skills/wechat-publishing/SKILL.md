...
- **转化区与内链一律用公众号地址（2026-08-31 用户定规，判废级）**：转化段、往期关联等文末区域**禁止博客链接与「阅读原文看完整版」类博客指向**（含 prepare 注入的「📖 完整原文」段）；文内互链一律用已发公众号文章地址（发表记录接口 `appmsgpublish?sub=list` 条目 `link` 字段的 `mp.weixin.qq.com/s/` 短链）；同系列未发过公众号时改挂同题材已发文。原稿 relref 解析不出公众号地址的降级为纯文本标题。细则见 blog-writing `references/wechat-retention.md` 转化层
- **转化段不进源稿**：「点在看/关注」转化文案**不写在 content/posts 源文件**（博客版错位）——推荐流程 = `make wechat-draft-only` 存草稿 → 后台编辑器在结论段后补价值锚定式转化段（单动作；禁诱导分享/强制关注）→ 群发；批量管线可在 prepare 与 publish_mp 之间对 `wechat-ready-weixin.html` 注入转化段
...
- **⚠️ 禁止定时「今天」（2026-08-05 事故）**：前端 dayChange 对「今天」降级为 `isFreePublish=true` 免费发布——文章**静默上主页、无粉丝推送**，走即时 masssend 不走 time_send，脚本 time_send 检测永远不触发 → 空响应「假失败真发布」（草稿 100001282 被免费发布上主页，挂太久无法撤回）。`schedule_ui_v2.py` 已加硬守卫 `is_today_label` 拒绝「今天/今日/当天日期」，**只允许定时未来配额日**（isFreePublish=false 群发通知）。真今天发需即时群发（暂不可靠）。教训：**空响应 ≠ 失败**，发布后查 home「近期发表」核对真实结果。 **已发文章删除与免费发表的直连 API 均不可靠（2026-08-31 codex-auto-video-editing 实证，用户手动操作才成功）**：① `appmsgpublish?sub=delete_submit` 带 `article_id` 返回 ret:0 + 完整 publish_page——看似成功实则**静默无效**（60s 后复验条目仍在），ret:0 是页面渲染不是删除动作，别信；② 免费发表 `masssend?is_release_publish_page=1&isFreePublish=true` 直连 ret:-1 真失败（同 time_send，缺弹窗态字段）。**两条都走后台 UI 手动操作**：发表记录→卡片「删除」；草稿→发表→次数用尽时选发布（不推送）。核验认 `appmsgpublish` 条目的 `is_deleted` 标志——API 列表会连同已删除条目一起返回，按「标题存在」判断会误判；且并发期用户手动操作与脚本动作要分清归因。
