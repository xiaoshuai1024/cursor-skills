...
- **⚠️ 禁止定时「今天」（2026-08-05 事故）**：前端 dayChange 对「今天」降级为 `isFreePublish=true` 免费发布——文章**静默上主页、无粉丝推送**，走即时 masssend 不走 time_send，脚本 time_send 检测永远不触发 → 空响应「假失败真发布」（草稿 100001282 被免费发布上主页，挂太久无法撤回）。`schedule_ui_v2.py` 已加硬守卫 `is_today_label` 拒绝「今天/今日/当天日期」，**只允许定时未来配额日**（isFreePublish=false 群发通知）。真今天发需即时群发（暂不可靠）。教训：**空响应 ≠ 失败**，发布后查 home「近期发表」核对真实结果。 另一实证（2026-08-31 codex-auto-video-editing）：免费发表 `masssend?t=ajax-response&is_release_publish_page=1&isFreePublish=true` 直连返回 ret:-1 但 15:52 实际发表成功；**核验发表记录必须认 `appmsgpublish` 条目的 `is_deleted` 标志——API 列表会连同已删除条目一起返回，按「标题存在」判断会误判删除/发表失败**；已发文章删除用 `appmsgpublish?sub=delete_submit` + form `article_id=<aid带_idx>`（带 Referer，ret:0 即生效）。
